...
const ProfileScreen = () => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        const response = await axios.get(
          `http://172.172.22.220:3000/api/users/profiles/${email}`,
          {
            headers: {
  ...
  const onSubmit: SubmitHandler<ProfileFormType> = async (
    ...
    try {
      ...
      // Add text fields (email and phoneNumber) as per API request body
      formData.append("firstName", "huy"); // Hardcoded for now, update as needed
      formData.append("lastName", "cuong"); // Hardcoded for now, update as needed
      formData.append("phoneNumber", data.phoneNumber);

      // Add image if it exists and has changed
      if (image && image !== originalValues.profileImage) {
        const fileName = image.split("/").pop() || "profile.jpg";
        const fileType = "image/jpeg"; // Adjust based on your image type
        formData.append("profileImage", {
          uri: image,
          name: fileName,
          type: fileType,
      ...
      const response = await axios.put(
        `http://172.172.22.220:3000/api/users/profiles/${data.email}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
